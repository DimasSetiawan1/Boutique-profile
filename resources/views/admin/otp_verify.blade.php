...
<head>
    ...
    <title>Verifikasi OTP Admin - Boutique Design Indonesia</title>
    <!-- Google Fonts -->
...
<body>
    ...
    <div class="login-card">
        ...
        <h4 class="fw-bold mb-2" style="color:#1e293b;">Verifikasi Kode OTP</h4>
        <p class="text-secondary small mb-4">
            Kode 6-digit telah dikirimkan ke email:<br>
            <strong class="text-dark">{{ session('otp_email') ?? 'email Anda' }}</strong>
        </p>
        ...
        <form action="{{ route('admin.account.otp.verify') }}" method="POST" id="otpForm">
            @csrf
            <div class="mb-4">
                <label for="otp" class="form-label small fw-bold text-secondary text-uppercase" style="letter-spacing: 1px;">Masukkan 6-Digit Kode</label>
                <input type="text" 
            ...
            <button type="submit" class="btn-verify mb-3">
                Verifikasi & Masuk <i class="bi bi-arrow-right-circle ms-1"></i>
            </button>
        </form>
        ...
        <div class="mt-4 pt-3 border-top">
            <a href="{{ route('admin.login') }}" class="back-btn">
                <i class="bi bi-arrow-left"></i> Gunakan akun / email lain
            </a>
        </div>
    </div>
